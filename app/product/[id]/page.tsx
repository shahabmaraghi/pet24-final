...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { reviews, addReview, rating, count } = useReviews(id);
  ...
  const onSubmitReview = (data: ReviewFormData) => {
    addReview(data.author, formRating, data.text);
    reviewForm.reset();
    setFormRating(5);
  };
    ...
    <>
      ...
      <div className="mb-14">
        ...
        <form onSubmit={reviewForm.handleSubmit(onSubmitReview)} className="mb-8 flex flex-col gap-3 rounded-2xl border bg-card p-5">
          ...
          <Button type="submit" className="self-start font-bold">ثبت نظر</Button>
        </form>
        {reviews.length > 0 && (
